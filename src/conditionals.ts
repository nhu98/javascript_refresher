...
//Compare the values of myAge and yourAge using if … else. Based on the comparison and log the result to console stating who is older (me or you). Use prompt(“Enter your age:”) to get the age as input.
  ...
  const myAgeInput: string = myAgeDefault
    ? myAgeDefault
    : prompt('Enter my age:')||'';
  const myAge: number = parseInt(myAgeInput);
  const yourAgeInput = yourAgeDefault
    ? yourAgeDefault
...
//ternary operator
  const compare: string =
    a > b ? 'a is greater than b' : a < b ? 'a is less than b' : 'a equal b';
  console.log(compare);
...
//Check if the season is Autumn, Winter, Spring or Summer. If the user input is
  ...
  const autumn: MonthsStrings[] = ['Septemper', 'October', 'November'];
  ...
  const monthName: MonthsStrings = monthDefault
    ? monthDefault
    : (prompt('Enter the month name?') as MonthsStrings);
...
//Check if a day is weekend day or a working day. Your script will take day as an input
  ...
  const dateName: WeekStrings = dateDefault
    ? dateDefault
    : (prompt('Enter day of week?') as WeekStrings);
...
//Write a program which tells the number of days in a month.
  ...
  const monthNameInput: MonthsStrings = monthDefault
    ? monthDefault
    : (prompt('Enter a month?') as MonthsStrings);
  ...
  switch (monthNameInput.toLowerCase()) {
    ...
    case Months.August.toLowerCase():
      console.log(`${monthNameInput} has 31 days.`);
      break;
    case Months.Septemper.toLowerCase():
      console.log(`${monthNameInput} has 30 days.`);
      break;
...
//Write a program which tells the number of days in a month, now consider leap year.
  ...
  const yearInput: number = yearDefault
    ? yearDefault
    : parseInt(prompt('Enter a year?')||'');
